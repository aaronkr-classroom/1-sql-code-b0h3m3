...
6. WHERE절의 LIKE 'g고’는 열 값으로 god, good, gd 등을 갖는 행들을 검색한다. (O)
7. NULL이 포함된 행은 대부분의 집계 함수의 결과에서 제외된다. (X)
...
2. SQL은 대화식 그리고 (내장형)의 2가지 방식으로 사용할 수 있다.
...
11. 중첩 질의문 중에서 내부 질의의 WHERE절 검색조건식이 외부 질의에 선언된 테이블의 일부 열을 참조하는 질의를 (상관서브쿼리)이라고 한다.
...
SELECT 학생.이름, SUM(중간성적 + 기말성적) AS 총점 FROM 학생 JOIN 수강 ON 학생.학번 = 수강.학번 GROUP BY 학생.이름;
